...
|---|---|---|---|---|
| C1 | Freeze writes on source (HPA disabled, deployments scaled-to-current) | `kubectl -n payments scale --replicas=$N` | No new ReplicaSets created | Re-enable HPA |
...
| C4 | Shift Route 53 weighted records: EKS=10, source=90 | `aws route53 change-resource-record-sets` | DNS resolves; ALB sees traffic | Set EKS weight to 0 |
| C5 | Watch ALB 4xx/5xx + p95 for 15 min | CloudWatch dashboard | Error rate < 0.1%; p95 within 10% of baseline | Step C4 rollback |
| C6 | Shift weights: EKS=50, source=50 | same as C4 | Same as C5 | Set EKS weight to 0 |
| C7 | Watch for 30 min | CloudWatch dashboard | Same as C5 | Step C4 rollback |
| C8 | Shift weights: EKS=100, source=0 | same as C4 | Same as C5 | Set EKS weight to 0 (source still warm) |
...
| # | Step | Owner | Verify |
|---|---|---|---|
| P1 | source replicas scaled to 0 (warm-cold) | Platform | `kubectl -n <ns> get deploy` on source |
| P2 | Run validation suite | SRE | See [`07-validation.md`](./07-validation.md) |
| P3 | App owners sign-off | App owners | Slack acknowledgement |
| P4 | Decommission source namespaces (delete, not stop) | Platform | `kubectl delete ns -l acme.io/wave=2` (GKE Enterprise cluster) |

## Rollback (any time within 24h)

1. Set Route 53 EKS weight = 0; source weight = 100.
2. Wait 60s for DNS TTL.
3. Re-enable HPA on source.
