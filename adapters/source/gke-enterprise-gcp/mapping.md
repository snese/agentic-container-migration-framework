# GKE Enterprise on GCP → AWS Feature Mapping

| GKE Enterprise / GKE Feature | AWS Equivalent (EKS) | AWS Equivalent (ECS) | Migration Notes |
|---|---|---|---|
...
| Config Sync (RootSync/RepoSync) | Flux v2 / Argo CD | — | Config Sync is GitOps; on AWS Flux is the standard; Argo CD if customer prefers |
...
| Anthos Service Mesh (managed Istio) | EKS + Istio (self-managed) | App Mesh / ECS Service Connect | Recommend Istio on EKS for like-for-like |
...
| GKE NetworkPolicies (Calico) | Calico on EKS / VPC CNI Network Policies | Security Groups / Service Connect policies | EKS supports NetworkPolicy via VPC CNI |
...
- **Autopilot pricing** is per-pod; EKS Auto Mode bills compute + a
  control-plane fee. Run an actual cost diff for portfolios with many small
  pods.
- **Workload Identity** annotation `iam.gke.io/gcp-service-account` → must
  rewrite as `eks.amazonaws.com/role-arn` for IRSA.
- **Anthos Service Mesh certs** are issued by Mesh CA; on AWS with Istio you
  bring your own root CA or use cert-manager.
