...
- Cluster-level discovery across Anthos, OpenShift, Rancher, vanilla K8s
- Migration Readiness Assessment (MRA) across all six AWS CAF perspectives
- Per-workload target-service selection (EKS vs ECS Fargate vs App Runner)
...
| 2 | Single legacy app already in a container, just needs to land on App Runner or ECS | **AWS Transform** | Per-app deploy automation, no portfolio decisions needed. |
| 3 | A 50-workload Anthos-on-VMware cluster moving to AWS | **ACMF** | Portfolio-level decisions (EKS vs ECS per workload, wave plan, landing zone) dominate. Transform doesn't address those. |
...
| 5 | A multi-cluster mixed estate (Anthos + Rancher + a few VMs) | **ACMF**, with Transform invoked inside the Migrate phase for individual apps that need containerization rework | Portfolio shape requires methodology; per-app rework benefits from Transform. |
...
ACMF Phase 2 (Mobilize)      ──→  Per-workload target decision (EKS/ECS/App Runner) + landing zone
...
- A specific service is being relocated to App Runner from a custom container build
- Per-app CI/CD scaffolding is needed and the customer doesn't have an existing pipeline

If the workload's 7 Rs decision is **Rehost** (manifest port — Anthos manifest → EKS manifest), Transform is usually unnecessary. The work is at the K8s manifest layer, which is ACMF's home turf.
