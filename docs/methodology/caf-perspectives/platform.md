...
Platform is the technical landing zone — accounts, networking, clusters, IaC, CI/CD plumbing. For container migrations, this is where ACMF spends the most opinionated effort, because the choice between EKS / ECS Fargate / App Runner / ROSA on AWS is not a one-size answer.
...
- **Multi-target reference architectures.** EKS (full K8s control), ECS Fargate (serverless containers), App Runner (single-service HTTP), ROSA on AWS (managed OpenShift). Each with a documented "happy path" landing zone.
...
- Per-target reference architecture (EKS / ECS Fargate / App Runner) as IaC modules in `iac-skeleton/`
...
- Copying Anthos / OpenShift opinions wholesale (e.g. cluster-per-team) into AWS without re-evaluating.

## How agentic discovery contributes

The discovery bundle drives target selection: workloads heavy on CRDs route to EKS; stateless HTTP services with simple needs surface as App Runner candidates; everything in between is ECS Fargate. Agent-generated IaC scaffolding (Terraform / CDK starter modules per workload) gives the platform team a working baseline instead of a blank repo on Day 1 of Mobilize.
