...
# aks-export.sh
#
# ACMF Phase 1 (Assess) — AKS (Azure Kubernetes Service) self-export.
#
# STATUS: 🟡 Stub — entry point + dry-run schema-valid bundle.
# Real-cluster mode runs shared kubectl collectors; AKS-specific
# enrichment (Azure AD RBAC, Azure CNI specifics, Workload Identity)
# is pending — see [STUB] markers.
#
# Requirements: bash 4+, kubectl, jq. Optional: az CLI.
...
OUTPUT=""
INCLUDE_NS=""
EXCLUDE_NS="kube-system,kube-public,kube-node-lease,gatekeeper-system,calico-system"
CLUSTER_NAME=""
...
[[ -z "$OUTPUT" ]] && OUTPUT="discovery-bundle-aks-$(date -u +%Y%m%dT%H%M%SZ).json"

acmf::common::init
trap 'rm -rf "$ACMF_TMPDIR"' EXIT

if [[ "$DRY_RUN" -eq 1 ]]; then
  acmf::common::log "Dry-run mode: emitting schema-valid stub bundle for AKS."
  acmf::bundle::dry_run_stub "$OUTPUT" "aks-export-script (dry-run)" "aks" "mock-aks-cluster" "azure" \
    "[STUB] aks-export: Azure AD RBAC + Workload Identity (azure.workload.identity/client-id) collection pending" \
    "[STUB] aks-export: az aks show enrichment (network plugin, sku tier, kubelet identity) pending"
...
CLUSTERS_JSON="$(jq -n --arg name "$CLUSTER_NAME" --arg ver "$K8S_VER" --argjson nc "${NODE_COUNT:-0}" \
  '[{ name:$name, version:$ver, location:"unknown", platform:"azure",
...
acmf::common::skip "az aks show" "[STUB] not yet wired — Azure CLI enrichment pending"
...
acmf::bundle::emit "$OUTPUT" "$(acmf::common::generated_at)" "aks-export-script" \
