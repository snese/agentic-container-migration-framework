...
# rancher-export.sh
#
# ACMF Phase 1 (Assess) — Rancher (or vanilla K8s) self-export.
#
# STATUS: 🟡 Stub — entry point + dry-run schema-valid bundle.
# Real-cluster mode runs shared kubectl collectors; Rancher-specific
# enrichment (Rancher projects, multi-cluster fleet, RKE/RKE2/k3s
# specifics) is pending.
#
# Requirements: bash 4+, kubectl, jq. Optional: rancher CLI.
...
OUTPUT=""
INCLUDE_NS=""
EXCLUDE_NS="kube-system,kube-public,kube-node-lease,cattle-system,cattle-fleet-system,cattle-impersonation-system,fleet-system,fleet-default,fleet-local"
CLUSTER_NAME=""
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  acmf::bundle::dry_run_stub "$OUTPUT" "rancher-export-script (dry-run)" "rancher" "mock-rancher-cluster" "other" \
    "[STUB] rancher-export: Rancher projects + project-role-binding collection pending - rancher CLI" \
    "[STUB] rancher-export: cluster-type detection (RKE/RKE2/k3s/imported) pending"
...
acmf::common::skip "rancher cluster ls" "[STUB] not yet wired - rancher CLI enrichment pending"
