...
# openshift-export.sh
#
# ACMF Phase 1 (Assess) — OpenShift (Red Hat OCP) self-export.
#
# STATUS: 🟡 Stub — entry point + dry-run schema-valid bundle.
# Real-cluster mode runs shared kubectl collectors; OpenShift-specific
# enrichment (OAuth, SCC, Routes, Templates, OperatorHub) is pending.
#
# Requirements: bash 4+, kubectl, jq. Optional: oc CLI.
...
OUTPUT=""
INCLUDE_NS=""
EXCLUDE_NS="kube-system,kube-public,kube-node-lease,openshift,openshift-apiserver,openshift-authentication,openshift-controller-manager,openshift-dns,openshift-etcd,openshift-ingress,openshift-monitoring,openshift-operators"
CLUSTER_NAME=""
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  acmf::bundle::dry_run_stub "$OUTPUT" "openshift-export-script (dry-run)" "openshift" "mock-ocp-cluster" "other" \
    "[STUB] openshift-export: OAuth identity provider + SCC binding collection pending - oc get oauth, oc get scc" \
    "[STUB] openshift-export: Routes, Templates, ImageStreams collection pending"
...
acmf::common::skip "oc get clusterversion" "[STUB] not yet wired - oc enrichment pending"
